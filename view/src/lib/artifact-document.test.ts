...
const restrictivePolicy = `default-src 'none'; img-src data: blob:; style-src 'unsafe-inline'; script-src 'unsafe-inline' ${tailwindBrowserCdn}; font-src data:; connect-src 'none'; form-action 'none'; base-uri 'none`;
...
describe("buildArtifactDocument", () => {
  it("turns a complete HTML document into a sandboxed preview without changing its markup", () => {
    ...
    const contentSecurityPolicy = document.querySelector('meta[http-equiv="Content-Security-Policy"]')?.getAttribute("content");

    expect(contentSecurityPolicy).toBe(restrictivePolicy);
    expect(contentSecurityPolicy).toContain(`script-src 'unsafe-inline' ${tailwindBrowserCdn};`);
    expect(contentSecurityPolicy).not.toContain("script-src 'unsafe-inline' https://cdn.jsdelivr.net;");
